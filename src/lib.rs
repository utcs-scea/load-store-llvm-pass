        ...
        for instr in b.get_instructions() {
          match instr.get_opcode() {
            InstructionOpcode::Load => {
              builder.position_at(b, &instr);
              let operand = instr.get_operand(0).unwrap().left().unwrap();
              eprintln!("(globalify-consts pass) -- LOAD -- operand 0 label name is {:?}", operand.get_name());

              // operand 0 is pointer/load location
              ...
              match operand {
                BasicValueEnum::PointerValue(ptr_val) => {
                  ...
                  eprintln!("(globalify-consts pass) -- LOAD -- check if it's const...");
                  // check if it is const. continue if not.
                  if !ptr_val.is_const() { continue; }

                  eprintln!("(globalify-consts pass) -- LOAD -- check if it has a label name...");
                  // check if it has a defined label name. continue if not.
                  if operand.get_name().to_str().unwrap() == "" { continue; }

                  // this is a const pointer with a defined label name. globalify it.
                  eprintln!("(globalify-consts pass) -- LOAD -- ptr + const + labelled. globalify it.");

                  // TODO:
                    // i THINK what i need to do is invoke the globalify function on the pointer, save the result.
                    // insert that as an instruction before the load.
                    // then change the operand of the load to use my new globalified value.

                    

                  // let replace_instr: InstructionValue = match builder
                  //   .build_direct_call(globalify_func, &[BasicMetadataValueEnum::PointerValue(ptr_val)], "globalify_func")
                  //   .unwrap()
                  //   .try_as_basic_value() {
                  //     Either::Left(basic_value) => {
                  //       if basic_value.is_pointer_value() {
            ...
            InstructionOpcode::Store => {
              ...
              // eprintln!("(globalify-consts pass) -- STORE -- operand 0 label name is {:?}", operand0.get_name());
              // eprintln!("(globalify-consts pass) -- STORE -- operand 1 label name is {:?}", operand1.get_name());

              // operand 1 is pointer/store location
            ...
            // InstructionOpcode::Alloca => {},   // not necessary I think?
            _ => {continue},
          };
        }
      }
